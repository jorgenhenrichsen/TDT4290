...
from .forms import UserCreationForm, UserLoginForm,UserChangePasswordForm,UserSetResetPasswordForm,UserCreationByAdminForm,UsersEditForm
...
def reset_password_mailer_view(request, *args,**kwargs):
    ...
    if form.is_valid():
        ...
        if security_qs.exists() and security_qs.count() == 1:
            ...
            msg = "Trykk på linken for å tilbakestille passord"
            html_message += "<h3> Trykk på linken for å tilbakestille passord <h3>"
            url = "http://127.0.0.1:8000/reset-password/"+ps_key
            html_message += '<a href="' + url + '"> Trykk her for å legge inn passord </a>'
